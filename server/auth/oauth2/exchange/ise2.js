...
/*
 * Exchanges ise2 (orange) id credentials for access tokens.
 *
 * This exchange middleware is used to by clients to obtain an access token by
 * presenting the resource owner's ise2 id credentials.
 *
 * Callbacks:
 *
 ...
 * token.  `id` is the ise2 identifier.
 ...
 * Examples:
 *
 *     server.exchange(oauth2orize.exchange.ise2(function(client, id, scope, done) {
 *       var user = ise2IdToUser(id);
...
module.exports = function (options, issue) {
  ...
  if (!issue) { throw new TypeError('oauth2.exchange.ise2 exchange requires an issue callback'); }
  ...
  return function ise2(req, res, next) {
    if (!req.body) { return next(new Error('oauth2.exchange.ise2 requires body parsing. Did you forget app.use(express.bodyParser())?')); }
